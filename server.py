...
def renderizar_pagina(conteudo_interno, **contexto):
    ...
    <head>
        ...
        <style>
            ...
            textarea { max-width: 100%; height: 140px; font-family: 'Courier New', Courier, monospace; }
            ...
            .btn-del { background: #b91c1c; padding: 4px 10px; font-size: 13px; color: #fff; border-radius:4px; text-decoration:none; } .btn-del:hover { background: #991b1b; }
            .btn-edit { background: #eab308; color: #000; padding: 4px 10px; font-size: 13px; border-radius:4px; text-decoration:none; font-weight:bold; } .btn-edit:hover { background: #ca8a04; }
            .btn-copy { background: #4b5563; padding: 4px 10px; font-size: 13px; color:#fff; border:none; border-radius:4px; cursor:pointer; } .btn-copy:hover { background: #374151; }
            .btn-rep { background: #ea580c; padding: 4px 10px; font-size: 13px; color:#fff; text-decoration:none; border-radius:4px; font-weight:bold; } .btn-rep:hover { background: #c2410c; }
    ...
    <body>
        <div class="container">
            <div class="navbar">
                ...
                {% else %}
                    <a href="/login" class="btn" style="background:#22c55e;">🔑 Entrar / Registrar</a>
                {% endif %}
            </div>
    """ + conteudo_interno + """
        </div>
...
def home():
    ...
    if supabase_client:
        try:
            ...
            for f in arquivos_storage:
                info = perm_dict.get(f, {"is_public": False, "tags": "Para local", "description": "", "owner": "admin", "is_featured": False})
    ...
    {% if destaques %}
    <h3 style="color:#eab308; text-align:left;">⭐ Scripts em Destaque Moderação</h3>
    <div class="grid-featured">
        ...
        <div class="card-featured">
            ...
            <div style="display:flex; gap:4px;">
                <button class="btn btn-copy" style="flex:1;" onclick="copiarTexto(window.location.origin + '/uploads/{{ arq.name }}')">Link</button>
                {% if current_user.is_authenticated and current_user.id != arq.owner %}
                    <a href="/report?user={{ arq.owner }}&file={{ arq.name }}" class="btn btn-rep" style="padding:4px 8px;">🚨</a>
                {% endif %}
                {% if current_user.is_authenticated and current_user.role == 'adm' %}
                    <a href="/edit-file/{{ arq.name }}" class="btn btn-edit" style="padding:4px 8px;">⚙️</a>
                {% endif %}
            </div>
        </div>
        {% endfor %}
        ...
        {% for arq in arquivos %}
            <div class="file-item">
                ...
                <div class="actions">
                    ...
                    {% if current_user.is_authenticated and current_user.id != arq.owner %}
                        <a href="/report?user={{ arq.owner }}&file={{ arq.name }}" class="btn btn-rep">Reportar</a>
                    {% endif %}
    ...
    {% for arq in arquivos %}
        <div class="file-item">
            ...
            <div class="actions">
                ...
                {% if current_user.is_authenticated and current_user.id != arq.owner %}
                    <a href="/report?user={{ arq.owner }}&file={{ arq.filename }}" class="btn btn-rep">Reportar</a>
                {% endif %}
                {% if current_user.is_authenticated and current_user.role == 'adm' %}
                    <a href="/delete/{{ arq.filename }}" class="btn btn-del">Excluir</a>
                {% endif %}
            </div>
        </div>
    {% endfor %}
...
# --- SISTEMA DE ENVIAR REPORTE (MODERAÇÃO) ---
...
def report_user():
    ...
    if request.method == 'POST':
        reported = request.form.get('reported_user')
        motivo = request.form.get('reason')
        arquivo_relacionado = request.form.get('filename')
        
        if supabase_client:
            try:
                texto_final_reporte = f"Arquivo: {arquivo_relacionado} | Motivo: {motivo}"
                supabase_client.table("reports_service").insert({
    ...
    <form method="POST">
        ...
        <label><b>Descreva detalhadamente o motivo da denúncia:</b></label><br>
        <textarea name="reason" placeholder="Ex: Este script contém comandos maliciosos escondidos / Descrição falsa..." required></textarea><br>
        <input type="submit" value="Enviar Denúncia" style="background:#ef4444;">
        <a href="/" class="btn" style="background:#4b5563;">Cancelar</a>
    </form>
    """, target=target_user, filename=target_file)
...
def panel():
    ...
    if supabase_client:
        if request.method == 'POST':
            ...
            if action == 'ban':
                supabase_client.table("users_service").update({"is_banned": True}).eq("username", target_user).execute()
                ...
                supabase_client.table("users_service").update({"session_version": 99}).eq("username", target_user).execute()
                msg_sucesso = f"Forçado encerramento de sessão de {target_user}!"
            elif action == 'clear_reports':
                supabase_client.table("reports_service").delete().eq("reported_user", target_user).execute()
                msg_sucesso = f"Histórico de reportes de {target_user} limpo!"
                ...
                <div style="display:flex; gap:5px; flex-wrap:wrap;">
                    <form method="POST" style="display:inline;">
                        ...
                        {% if u.is_banned %}
                            ...
                            <input type="submit" value="Desbanir" style="background:#22c55e; padding:4px 8px; font-size:12px; border:none; border-radius:4px; color:#fff; cursor:pointer;">
                        {% else %}
                            <input type="hidden" name="action" value="ban">
                            <input type="submit" value="Banir" style="background:#ef4444; padding:4px 8px; font-size:12px; border:none; border-radius:4px; color:#fff; cursor:pointer;">
                        {% endif %}
                    </form>
                    <form method="POST" style="display:inline;">
                        <input type="hidden" name="target_user" value="{{ u.username }}">
                        <input type="hidden" name="action" value="logout_user">
                        <input type="submit" value="Deslogar" style="background:#f97316; padding:4px 8px; font-size:12px; border:none; border-radius:4px; color:#fff; cursor:pointer;">
                    </form>
                    <button class="btn" style="padding:4px 8px; font-size:12px; background:#4b5563; margin:0;" onclick="redefinirSenha('{{ u.username }}')">Senha</button>
                </div>
            </div>
        {% endfor %}
    </div>
        ...
        {% for rep in reports %}
            <div style="border-bottom: 1px solid #333; padding: 8px 0;">
                ...
                <form method="POST" style="margin-top:4px;">
                    ...
                    <input type="submit" value="Limpar Reporte" style="background:#4b5563; border:none; color:#fff; padding:2px 6px; font-size:11px; cursor:pointer; border-radius:3px;">
                </form>
            </div>
        {% else %}
            <p style="color:#888; text-align:center;">Nenhum reporte aberto na fila de moderação.</p>
        {% endfor %}
    </div>
...
def edit_file(filename):
    ...
    try:
        ...
        conteudo_arquivo = "-- Arquivo de dados ou sem suporte a texto."
